...
use crate::{error::LSHError, symbol_table::SymbolTable};
    ...
    fn from(value: Pair<'_, Rule>) -> Self {
        match value.as_rule() {
            ...
            Rule::lambda => {
                ...
                Expression::Function(Function::Defined(LambdaFunction { params , body }))
            },
            Rule::r#macro => {
                let mut value = value.into_inner();
                let params = Expression::from(value.next().unwrap()).get_symbol_list().clone();
